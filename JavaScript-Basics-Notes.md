...
| Topic | Key idea |
|-------|----------|
| Variables | let (reassign), const (don’t reassign) |
| Strings | Index from 0, .length, .slice(), .split(), etc. |
| typeof | "string", "number", "boolean", "undefined", "object" |
| Conversions | Number(), String(), parseInt(), parseFloat() |
| Template strings | Backticks and ${ } |
| Comparisons | Prefer === and !== |
| Truthy / Falsy | false, 0, "", null, undefined, NaN are falsy |
| Ternary | condition ? a : b |
| && / \|\| | And / Or; also used for defaults (e.g. name \|\| "Guest") |
| Switch | break needed to avoid fall-through |
| Destructuring | { a, b } = obj; [x, y] = arr; |
| Rest | ...rest in params or destructuring |
| Callback | Function passed to another to run later |
| forEach | Run function per item; no return |
| for in / for of | Keys vs values |
| map | New array from function on each item |
| filter | New array of items that pass test |
| reduce | One value from array (acc, item) => ... |
| sort | In place; use (a,b) => a-b for numbers |
| find | First item that passes test |
| every / some | All pass? / At least one passes? |
| fill / splice | Fill with value / Add or remove at index |
| DOM | Tree of HTML as objects; document, getElementById, querySelector |
| textContent / innerText | Get/set text |
| style | el.style.backgroundColor, etc. (camelCase) |
| querySelectorAll | NodeList of all matches |
| innerHTML | Get/set HTML (watch XSS) |
| setTimeout / setInterval | Run once after delay / run repeatedly; clear with clearTimeout/clearInterval |

Use this file as a cheat sheet while you code. Try each concept in the browser console or in a small .js file.
